...
func checkFreshnessNoAnchor(ctx context.Context, store, n string) Result {
	e, err := cairn.NewEntry(cairn.NewEntryParams{TopicKey: "critic-freshness-no-anchor-" + n, Body: "no-anchor fixture body", CreatedBy: "critic"})
	if err != nil {
...
func checkFreshnessNeverVerified(ctx context.Context, store, n, repo string) Result {
	e, err := cairn.NewEntry(cairn.NewEntryParams{TopicKey: "critic-freshness-never-verified-" + n, Body: "never-verified fixture body", CreatedBy: "critic"})
	if err != nil {
		return NewResult(DimensionFreshness, freshnessScenarioID, Fail, fmt.Sprintf("build entry: %v", err))
	}
...
func checkFreshnessInvocationIncomplete(ctx context.Context, store, n, repo string) Result {
	e, err := cairn.NewEntry(cairn.NewEntryParams{TopicKey: "critic-freshness-invocation-incomplete-" + n, Body: "invocation-incomplete fixture body", CreatedBy: "critic"})
	if err != nil {
		return NewResult(DimensionFreshness, freshnessScenarioID, Fail, fmt.Sprintf("build entry: %v", err))
	}
	e.Anchor = cairn.Anchor{Type: "files", Repo: repo, Paths: []string{freshnessFixtureFile}}
