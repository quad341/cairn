	...
	for _, e := range batch {
		...
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO entries (
				...
				overridden_duplicate_of, review_status, corrects
			) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
			...
			// overridden_duplicate_of, review_status, and corrects are body-sourced
			// (like created_at), not index-only, so unlike those they ARE in the
			// UPDATE SET -- a --force correction, a review-status flip, or a
			// corrects edit made into the body must overwrite a stale indexed copy.
			...
			e.OverriddenDuplicateOf, e.ReviewStatus, e.Corrects,
		); err != nil {
			_ = tx.Rollback()
			return err
		}
