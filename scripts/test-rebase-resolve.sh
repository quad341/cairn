#!/usr/bin/env bash
#
# test-rebase-resolve.sh — unit tests for the deployer's git-safety lib
# (scripts/rebase-resolve-lib.sh). Two features, four layers:
#
...
#      refuse. These cases were originally PORTED from
#      packs/maintainer-pr-review/tests/test-rebase-resolve.sh into
#      gc-management's packs/actual/deployer/tests/test-rebase-resolve.sh,
#      because the classifier functions under test
...
#      ported copy — see scripts/rebase-resolve-lib.sh's header. This file is
#      cairn's own copy of gc-management's packs/actual/deployer copy
#      (ported crn-elja.1); do not hand-edit it to "sync" with the
#      gc-management original — port forward from there instead.
#      (bead ga-gcy0cd; architecture ga-h7hnpt FR-5/FR-6.)
#   2. attempt_bounded_self_rebase (deployer driver, not in the mpr copy):
...
#      --force).
#   3. Static guards: the lib's push must use --force-with-lease and must
#      never use a bare --force.
#   4. Deploy branch-target safety (bead crn-wya): is_shared_worktree_branch
#      classification (gc-<agent>-<12hex> — incl. dashed agent names and
#      ref/owner-prefixed forms — vs the isolated deploy//release/ branches),
#      assert_safe_push_target refusal, and an executable incident-replay of
#      resolve_deploy_branch_target proving the cut deploy branch carries ONLY
#      the reviewed SHA's ancestry while the shared branch is left untouched.
#      (gc-management's own copy of this suite additionally carries static
#      assertions that its deployer prompt + gate formula templates derive
#      their push target from the bead-id/SHA — those check
#      prompts/deployer.md.tmpl and formulas/mol-deployer-gate.formula.toml,
#      which are gc-management pack-template files with no counterpart in
#      this repo, so they are dropped here.)
#
...
PACK_DIR="$(cd "$TEST_DIR/.." && pwd)"
LIB="$PACK_DIR/scripts/rebase-resolve-lib.sh"
...
# ---------------------------------------------------------------------------
# Deploy branch-target safety (crn-wya) — is_shared_worktree_branch /
# assert_safe_push_target / resolve_deploy_branch_target, plus static
# assertions on the deployer prompt + gate formula.
# ---------------------------------------------------------------------------

test_shared_worktree_branch_classification() {
    local p
    # Positive: the exact shape worktree-setup.sh branch_name() emits, incl.
    # dashed agent names and ref/owner-prefixed forms a push/PR-head can take.
    for p in \
        "gc-builder-6ac3e0f3c1f3" \
        "gc-architect-397426e44c99" \
        "gc-deployer-0686f1163d9a" \
        "gc-planner-6dcb0a122f04" \
        "gc-gemini-reviewer-6ac3e0f3c1f3" \
        "gc-maintainer-pr-review-0123456789ab" \
        "gc-builder-1-6ac3e0f3c1f3" \
        "gc-all.deployer-6ac3e0f3c1f3" \
        "gc-gastown.capable-0123456789ab" \
        "gc-pack-author.pack-author-fe5274c1dc10" \
        "refs/heads/gc-builder-6ac3e0f3c1f3" \
        "origin/gc-all.deployer-6ac3e0f3c1f3" \
        "quad341:gc-builder-6ac3e0f3c1f3" \
        "quad341:refs/heads/gc-builder-6ac3e0f3c1f3"
    do
        if ! is_shared_worktree_branch "$p"; then
            record_fail "shared-branch/$p" "expected SHARED (match), got non-shared"
            return
        fi
    done
    # Negative: isolated deploy//release/ branches, plain names, and near-miss
    # signatures (11 hex, 13 chars, non-hex) must all read as safe.
    for p in \
        "deploy/crn-1qb-gate" \
        "deploy/crn-4bd.2.1-gate" \
        "release/gm-3fx1b2" \
        "main" \
        "master" \
        "feature/foo" \
        "quad341:deploy/crn-1qb-gate" \
        "gc-builder-6ac3e0f3c1f" \
        "gc-builder-6ac3e0f3c1f3a" \
        "gc-builder-6ac3e0f3c1fg" \
        "gc-builder" \
        ""
    do
        if is_shared_worktree_branch "$p"; then
            record_fail "shared-branch/'$p'" "expected NON-shared (safe), got shared"
            return
        fi
    done
    record_pass "shared-branch classification (gc-<agent>-<12hex> incl. dashed/dotted/numbered agents + ref/owner prefixes yes; deploy//release//near-miss no)"
}

test_assert_safe_push_target() {
    local rc
    assert_safe_push_target "gc-builder-6ac3e0f3c1f3" >/dev/null 2>&1; rc=$?
    if [[ $rc -eq 0 ]]; then record_fail "assert/shared-refused" "expected nonzero for shared branch"; return; fi
    assert_safe_push_target "quad341:gc-builder-6ac3e0f3c1f3" >/dev/null 2>&1; rc=$?
    if [[ $rc -eq 0 ]]; then record_fail "assert/shared-fork-head-refused" "expected nonzero for owner:shared head-ref"; return; fi
    assert_safe_push_target "" >/dev/null 2>&1; rc=$?
    if [[ $rc -eq 0 ]]; then record_fail "assert/empty-refused" "expected nonzero for empty target"; return; fi
    assert_safe_push_target "deploy/crn-1qb-gate" >/dev/null 2>&1; rc=$?
    if [[ $rc -ne 0 ]]; then record_fail "assert/deploy-allowed" "expected zero for deploy/ branch, got $rc"; return; fi
    assert_safe_push_target "release/gm-3fx1b2" >/dev/null 2>&1; rc=$?
    if [[ $rc -ne 0 ]]; then record_fail "assert/release-allowed" "expected zero for release/ branch, got $rc"; return; fi
    record_pass "assert_safe_push_target (shared + owner:shared + empty refused; deploy//release/ allowed)"
}

# The core incident replay: a reviewed commit on a SHARED builder branch, then
# unreviewed concurrent commits land on top (exactly the crn-wya / cairn#3
# shape). resolve_deploy_branch_target must cut an isolated branch that carries
# ONLY the reviewed SHA's ancestry, leave the shared branch untouched, and the
# guard must refuse a push to the shared name.
test_resolve_deploy_branch_target_incident_replay() {
    local d; d="$(new_repo)"
    local reviewed_sha shared_tip out rc
    (
        cd "$d" || exit 1
        printf 'base\n' > base.txt; git add -A; git commit -qm base
        # Shared, persistent per-role builder worktree branch — the exact name
        # shape worktree-setup.sh emits (gc-<agent>-<12hex>).
        git checkout -q -b gc-builder-6ac3e0f3c1f3
        printf 'reviewed change\n' > reviewed.txt; git add -A; git commit -qm "crn-9l6 reviewed work"
    )
    reviewed_sha="$(git -C "$d" rev-parse HEAD)"
    (
        cd "$d" || exit 1
        # Concurrent, UNREVIEWED work lands on the same shared branch after
        # review — the incident's 284ea1d / 45b2623 / cc5bca2.
        printf 'u1\n' > u1.txt; git add -A; git commit -qm "concurrent u1"
        printf 'u2\n' > u2.txt; git add -A; git commit -qm "concurrent u2"
        printf 'u3\n' > u3.txt; git add -A; git commit -qm "concurrent u3 (exists nowhere else)"
    )
    shared_tip="$(git -C "$d" rev-parse gc-builder-6ac3e0f3c1f3)"

    out="$(cd "$d" && resolve_deploy_branch_target "crn-1qb" "$reviewed_sha")"; rc=$?

    local deploy_sha cur_branch anc_rc shared_rc deploy_rc shared_after
    deploy_sha="$(git -C "$d" rev-parse -q --verify 'refs/heads/deploy/crn-1qb-gate' 2>/dev/null || true)"
    cur_branch="$(git -C "$d" rev-parse --abbrev-ref HEAD 2>/dev/null || true)"
    git -C "$d" merge-base --is-ancestor "$shared_tip" "deploy/crn-1qb-gate" 2>/dev/null; anc_rc=$?
    shared_after="$(git -C "$d" rev-parse gc-builder-6ac3e0f3c1f3 2>/dev/null || true)"
    assert_safe_push_target "gc-builder-6ac3e0f3c1f3" >/dev/null 2>&1; shared_rc=$?
    assert_safe_push_target "deploy/crn-1qb-gate" >/dev/null 2>&1; deploy_rc=$?

    if [[ $rc -eq 0 \
          && "$out" == "deploy/crn-1qb-gate" \
          && "$deploy_sha" == "$reviewed_sha" \
          && "$cur_branch" == "deploy/crn-1qb-gate" \
          && $anc_rc -ne 0 \
          && ! -f "$d/u3.txt" \
          && "$shared_after" == "$shared_tip" \
          && $shared_rc -ne 0 \
          && $deploy_rc -eq 0 ]]; then
        record_pass "resolve/incident-replay (deploy branch = reviewed SHA only; shared branch untouched; shared push refused)"
    else
        record_fail "resolve/incident-replay" \
            "rc=$rc out=$out deploy_sha=$deploy_sha reviewed_sha=$reviewed_sha cur=$cur_branch anc_rc=$anc_rc u3=$( [[ -f "$d/u3.txt" ]] && echo present || echo absent ) shared_after=$shared_after shared_tip=$shared_tip shared_rc=$shared_rc deploy_rc=$deploy_rc"
    fi
    rm -rf "$d"
}
...
test_resolve_deploy_branch_target_rejects_bad_input() {
    local d; d="$(new_repo)"
    local good_sha rc_empty rc_badsha rc_badid
    ( cd "$d" && printf 'x\n' > f.txt && git add -A && git commit -qm base )
    good_sha="$(git -C "$d" rev-parse HEAD)"
    ( cd "$d" && resolve_deploy_branch_target "" "$good_sha" >/dev/null 2>&1 ); rc_empty=$?
    ( cd "$d" && resolve_deploy_branch_target "crn-1qb" "0000000000000000000000000000000000000000" >/dev/null 2>&1 ); rc_badsha=$?
    ( cd "$d" && resolve_deploy_branch_target "bad/id" "$good_sha" >/dev/null 2>&1 ); rc_badid=$?
    if [[ $rc_empty -eq 10 && $rc_badsha -eq 10 && $rc_badid -eq 10 ]]; then
        record_pass "resolve/rejects-bad-input (empty bead-id, unresolvable SHA, slash in bead-id -> rc 10)"
    else
        record_fail "resolve/rejects-bad-input" "rc_empty=$rc_empty rc_badsha=$rc_badsha rc_badid=$rc_badid"
    fi
    rm -rf "$d"
}

test_static_shared_branch_rejection_pattern_present() {
    # The shared-branch signature the guard rejects must be present in the lib.
    if grep -qF 'gc-[A-Za-z0-9._-]+-[0-9a-f]{12}' "$LIB"; then
        record_pass "static/shared-branch-rejection-pattern-present"
    else
        record_fail "static/shared-branch-rejection-pattern-present" "signature regex not found in $LIB"
    fi
}

# test_static_deployer_prompt_derives_deploy_branch and
# test_static_formula_derives_deploy_branch from gc-management's copy of this
# suite are intentionally NOT ported here: they assert against
# prompts/deployer.md.tmpl and formulas/mol-deployer-gate.formula.toml, which
# are gc-management pack-template files rendered into cairn's live deployer
# role — this repo has no prompts/ or formulas/ directory for them to check
# (crn-elja.1).
